...
from mcp.client.streamable_http import strea
mablehttp_client
...
def add_to_memory(session_id, role, content):
    # Use Memory Client to store
    # Construct message object
    message = {"role": role, "content": content}
    memory_client.add_memory(session_id, [message])
    print(f"💾 Saved to memory [{session_id}]: {role} - {len(str(content))} chars")

def get_memory(session_id, limit=10):
    # Retrieve from Memory Client
    try:
        msgs = memory_client.get_memory(session_id, limit=limit)
        return msgs
...
# --- Lambda Handler ---
# Required for AWS Lambda deployment using Mangum
